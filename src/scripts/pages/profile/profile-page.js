...
export default class ProfilePage {
  ...
  #setupProfileUpdatedListener() {
    ...
    document.addEventListener("profileUpdated", this.#profileUpdatedHandler);
  }
      ...
      try {
        if (this.#presenter) {
          ...
          if (result && !result.error) {
            ...
            alert("Gagal memperbarui profil. Respons tidak diketahui.");
          }
        }
      } catch (error) {
  ...
  showUserStories(stories) {
    ...
    if (!storiesContainer) {
      console.error("Element with ID 'user-stories-container' not found.");
      return;
    }
    ...
    storiesContainer.innerHTML = stories
      .map((story) => {
        ...
        const isOwner =
          !story.isAnonymous && story.username === this.#currentUser.username;
  ...
  updateProfileHeaderDisplay(userData) {
    ...
    if (profilePicElement)
      profilePicElement.src = userData.profilePicture || "/images/image.png";
    if (nameElement) nameElement.textContent = userData.name || "Nama Pengguna";
    if (usernameElement)
