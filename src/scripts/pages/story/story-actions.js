export const setupStoryActions = (
    ...
    container.addEventListener('click', async (e) => {
        ...
        if (editBtn) {
            ...
            if (!storyContainer) {
                console.error(
                    'Story container not found for edit button:',
                    editBtn
                );
                return;
            }
            ...
            if (confirm('Apakah Anda yakin ingin menghapus cerita ini?')) {
                try {
                    ...
                    if (response && !response.error) {
                        ...
                        throw new Error(
                            response?.message || 'Gagal menghapus cerita'
                        );
                    }
                } catch (error) {
    ...
    function showSuccessToast(message) {
        ...
        toastElement.className =
            'fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-4 py-2 rounded shadow-lg z-50';
