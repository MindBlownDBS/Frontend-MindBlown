...
export default class StoryDetailPage {
    ...
    async render() {
        ...
        const profilePicture =
            currentUser?.profilePicture || './images/image.png';
    ...
    _setupStoryDataChangedListener() {
        ...
        this._storyDataChangedHandler = (event) => {
            const { action, entityId, userLiked, newLikeCount, message } =
                event.detail;

            if (action === 'commentLiked' && entityId) {
                const commentElement = document.querySelector(
                    `.comment-item-container[data-comment-id="${entityId}"]`
                );

                if (commentElement) {
                    const likeBtn =
                        commentElement.querySelector('.comment-like-btn');
                    const heartSvg = likeBtn?.querySelector('svg');
                    const likeCountElement = likeBtn?.querySelector(
                        '.comment-like-count'
                    );
                    ...
                    if (likeBtn && heartSvg && likeCountElement) {
                        ...
                        console.warn(
                            `⚠️ Could not find all required elements for comment ${entityId}`
                        );
                    }
                } else {
                    console.warn(
                        `⚠️ Comment element with ID ${entityId} not found`
                    );
                }
            }
    ...
    showStoryDetail(story) {
        ...
        const isOwner =
            !story.isAnonymous &&
            story.username === currentUserFromStorage.username;
        ...
        if (!commentsListContainer) {
            console.error(
                "Element 'comments-list' not found in StoryDetailPage."
            );
            return;
        }
    ...
    showComments(comments) {
        ...
        if (container) {
            ...
            container.innerHTML = comments
                .map((comment) => {
                    const isOwner =
                        this._currentUser?.username === comment.username;

                    return commentItemTemplate({
                        commentId: comment._id || comment.id,
                        username:
                            comment.name || comment.username || 'Pengguna',
                        handle: comment.username
                            ? `@${comment.username}`
                            : 'Anonim',
                        content: comment.content,
            ...
            try {
                if (this._presenter) {
                    ...
                    if (success) {
                        ...
                        if (
                            mobileModal &&
                            mobileModal.classList.contains('flex')
                        ) {
                            mobileModal.classList.remove('flex');
                            mobileModal.classList.add('hidden');
                        }
