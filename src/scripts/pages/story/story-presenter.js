...
export default class StoryPresenter {
    ...
    async getCompleteUserData(username) {
        try {
            ...
            return {
                ...
                profilePicture:
                    response.data?.profilePicture || './images/image.png',
            };
        } catch (error) {
            console.error('Error getting user data:', error);
            return null;
    ...
    async loadStoryDetail(storyId) {
        try {
            ...
            if (!response || response.error) {
                const errorMsg =
                    response?.message || 'Failed to load story details';
                console.error('Error loading story detail:', errorMsg);
                this._view.showError(
                    'Gagal memuat detail story. Silakan coba lagi.'
                );
                return;
            }
            ...
            if (story.comments && story.comments.length > 0) {
                for (const comment of story.comments) {
                    ...
                    if (comment.username) {
                        ...
                        const commenterData =
                            await this.getCompleteUserData(originalHandle);

                        comment.profilePicture =
                            commenterData?.profilePicture ||
                            './images/image.png';
                        comment.displayName =
                            commenterData?.name || originalHandle;
    ...
    async loadCommentDetail(commentId) {
        try {
            ...
            this._view.showError(
                error.message ||
                    'Terjadi kesalahan saat memuat detail komentar.'
            );
        }
    }
    ...
    async addReplyToComment(parentCommentId, content) {
        try {
            ...
            if (response.error) {
                throw new Error(
                    response.message || 'Gagal menambahkan balasan.'
                );
            }
            document.dispatchEvent(
    ...
    async deleteExistingComment(commentId) {
        try {
            ...
            const currentCommentId = isCommentDetailPage
