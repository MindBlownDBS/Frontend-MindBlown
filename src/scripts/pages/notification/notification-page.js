...
export default class NotificationPage {
        ...
        try {
            if (action === 'subscribe') {
                ...
                if (subscribeSuccess) {
                    ...
                    btn.className =
                        'px-4 py-2 bg-red-500 text-white text-sm rounded-lg hover:bg-red-600 transition-colors';
                    btn.dataset.action = 'unsubscribe';
                } else {
                    btn.textContent = 'Aktifkan Push Notifikasi';
                    btn.className =
                        'px-4 py-2 bg-third text-white text-sm rounded-lg hover:bg-third/90 transition-colors';
            ...
            if (action === 'subscribe') {
                ...
                alert(
                    'Gagal menonaktifkan push notification: ' + error.message
                );
            }
    ...
    renderCurrentPage() {
        ...
        const listContainer = document.getElementById('notification-list');
        listContainer.innerHTML =
            notificationListTemplate(currentNotifications);
    ...
    attachNotificationClickListeners() {
        ...
        notificationItems.forEach((item) => {
            item.removeEventListener(
                'click',
                this.handleNotificationClickBound
            );
        ...
        if (notification) {
            ...
            if (wasUnread) {
                this.presenter.markAsRead(notificationId).catch((error) => {
                    console.warn(
                        'Failed to mark notification as read on server, but UI updated locally:',
                        error
                    );
                });
            }
    ...
    updateMarkAllButtonVisibility() {
        const markAllBtn = document.getElementById('mark-all-read-btn');
        if (markAllBtn) {
            const hasUnreadNotifications = this.notifications.some(
                (n) => !n.read
            );
