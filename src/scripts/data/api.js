...
export async function checkTodayEntry() {
    ...
    if (response.error) {
        throw new Error(
            result.message || 'Terjadi kesalahan saat mengecek data'
        );
    }
...
export async function getMonthlyTrackerEntries(monthOffset = 0) {
    try {
        ...
        if (!response.ok) {
            throw new Error(
                result.message ||
                    'Terjadi kesalahan saat mengambil data tracker bulanan'
            );
        }
...
export async function markNotificationAsRead(notificationId) {
    try {
        ...
        return {
            error: true,
            message:
                error.message || 'Gagal menandai notifikasi sebagai dibaca',
        };
    }
}
...
export async function subscribePushNotification(subscription) {
    try {
        ...
        if (!response.ok) {
            throw new Error(
                result.message ||
                    'Gagal menyimpan subscription push notification'
            );
        }
...
export async function unsubscribePushNotification() {
    try {
        ...
        const response = await fetch(
            `${BASE_URL}${ENDPOINTS.UNSUBSCRIBE_PUSH}`,
            {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${accessToken}`,
        ...
        if (!response.ok) {
            throw new Error(
                result.message ||
                    'Gagal menghapus subscription push notification'
            );
        }
        ...
        return {
            ...
            message:
                error.message ||
                'Gagal berhenti berlangganan push notification',
        };
    }
}
...
export const editStory = async (storyId, content) => {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(
                data.message ||
                    `Failed to edit story. Status: ${response.status}`
            );
        }
        return data;
    } catch (error) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                console.warn(
                    'Could not parse error response as JSON for deleteStory'
                );
            }
            console.error(
...
export async function getCommentDetail(commentId) {
    ...
    if (!response.ok) {
        throw new Error(
            `Failed to fetch comment detail: ${response.statusText}`
        );
    }
    return await response.json();
}
...
export async function getUserRecommendations(username) {
    try {
        ...
        const response = await fetch(
            `${BASE_URL}${ENDPOINTS.RECOMMENDATIONS}/${username}`,
            {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
        ...
        if (!response.ok) {
            throw new Error(
                result.message ||
                    `Error ${response.status}: Terjadi kesalahan saat mengambil rekomendasi`
            );
        }
...
export async function regenerateRecommendations(username) {
    try {
        ...
        const response = await fetch(
            `${BASE_URL}${ENDPOINTS.RECOMMENDATIONS}/${username}/regenerate`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
        ...
        if (!response.ok) {
            throw new Error(
                result.message ||
                    `Error ${response.status}: Terjadi kesalahan saat meregenerasi rekomendasi`
            );
        }
        ...
        return {
            error: true,
            message:
                error.message || 'Gagal meregenerasi rekomendasi aktivitas',
        };
    }
}
...
export async function getChatHistory() {
    try {
        ...
        const response = await fetch(
            `${BASE_URL}${ENDPOINTS.CHATBOT_HISTORY}`,
            {
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                },
        ...
        if (!response.ok) {
            ...
            return {
                ...
                message:
                    result.message ||
                    `Error ${response.status}: Terjadi kesalahan saat mengambil riwayat chat`,
                data: { chats: [], total: 0 },
            };
        }
